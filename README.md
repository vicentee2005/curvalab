...
facilidad de uso. La matemática del ajuste corre en Python (SciPy/NumPy/pandas)
y las gráficas 2D/3D en Plotly.
...
│  │  └─ importer.py   Importación de CSV/TXT/XLSX (pandas)
...
El repositorio está preparado para desplegarse tal cual: el frontend se compila
como sitio estático y **el backend entero se convierte en una función
serverless** (`api/index.py` reexporta la misma app de FastAPI que se usa en
local, así que el motor de cálculo es idéntico en los dos sitios).
...
| `requirements.txt` (raíz) | Dependencias de la función, **sin pandas ni uvicorn** |
| `package.json` (raíz) | `npm run build` → compila el frontend |

**Por qué no hay pandas:** una función serverless de Vercel no puede pasar de
250 MB descomprimidos, y `numpy + scipy + sympy + pandas` se va a unos 265 MB.
El importador de CSV/Excel se reescribió con `openpyxl` y el módulo `csv` de la
biblioteca estándar (mismos tests, y además ahora avisa con un mensaje claro si
le das un `.xls` antiguo). Solo los generadores de `datos_ejemplo/` siguen
usando pandas, y esos no se despliegan.

En local, el frontend llama a `http://localhost:8000`; desplegado, la API vive
en el mismo origen y la base es la cadena vacía. Se decide en `frontend/src/api.ts`
con `import.meta.env.DEV`.

> **Privacidad:** en la versión desplegada los números que escribes viajan a la
> función serverless para calcularse. No hay cuentas ni base de datos y no se
> guarda nada, pero la promesa de «los datos no salen de tu ordenador» solo vale
